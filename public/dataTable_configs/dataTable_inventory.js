(function($) {
        ...
        buttons: [
            {
                ...
                customize: function(xlsx) {
                    ...
                    $('row c[r^="A1"]', sheet).each( function () {
                        if ( $('is t', this).text() == 'ACAI' ) {
                            $('is t', this).text( 'Inventario - ACAI '+ dateString );
                        } else {
                            $('is t', this).text( 'Inventario - ACAI'+ dateString );
                        }
                    });
                ...
                filename: function() {
                    ...
                    return 'Inventario - ' + day + ' de ' + month + ' de ' + year;
                },
                exportOptions: {
                    modifier: {
                        search: 'applied',
   ...
   // Script to show a filter per column, excluding 4 last columns
    $("#example tfoot th").each(function(index) {
        if (index < $("#example thead th").length - 4) {
            var title = $("#example thead th").eq($(this).index()).text();
            $(this).html('<input type="text" data-kt-filter="search" style="width: 100%;" placeholder="Filtrar ' + title + '" />');
        }
    });

    // Apply the filter, adjusting for the excluded first column
